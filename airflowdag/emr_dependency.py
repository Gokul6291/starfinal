...
def create_cluster(region_name, cluster_name='gokul_Cluster_' + str(datetime.now()), release_label='emr-6.2.1',master_instance_type='m5.xlarge', num_core_nodes=2, core_node_instance_type='m5.xlarge'):
        ...
        BootstrapActions= [
                {
                    ...
                    'ScriptBootstrapAction': {
                            'Path': 's3://grlandingzone/new_dependencies.sh',
                        }
                }
            ],
...
def track_statement_progress(master_dns, response_headers):
    ...
    while statement_status != 'success':
        ...
        if statement_status == 'dead':
            raise AirflowException('Final Statement Status: ' + statement_status)
